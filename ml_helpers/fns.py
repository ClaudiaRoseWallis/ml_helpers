...
def label_file_summary(df, id_col = 'id', label_col = 'label',
                       date_col = 'snap_d', weight_col = 'weight', 
                       outcome_class = 1, monthly_f = False,
                       weights = False, time_var = False):
    # save function start time to calc function time later
    start_time = time.time()
    
    # copy df
    label_file_df = df.copy()
    
    # get counts per months
    ## where monthly_f = True
    if monthly_f:
      ### create col with month & year of date_col
      label_file_df['year_month'] =\
      pd.to_datetime(label_file_df[date_col]).dt.to_period('M')
      
      ### get count of volumes per month
      label_file_dfc = label_file_df.copy() # copy to avoid warning
      label_file_dfc['volume'] =\
      label_file_dfc.groupby('year_month')[id_col].transform('count')
      
      ### get avg weight per month
      if weights:
        label_file_dfc['average_weight'] =\
        label_file_dfc.groupby('year_month')[weight_col].transform('mean')
        
      ### get count of volumes per month & class
      label_file_dfc['outcome_volume'] =\
      label_file_dfc.groupby(['year_month',
                              label_col])[id_col].transform('count')]
      
      ### calculate outcome rate per month & class
      label_file_dfc['outcome_rate'] =\
      label_file_dfc['outcome_volume'] / label_file_dfc['volume'] * 100
      
      ### get count of weighted volumes & outcomes per month & class
      if weights:
        label_file_dfc['volume_w'] =\
        label_file_dfc.groupby('year_month')[weight_col].\
        transform('sum')
        
        label_file_dfc['outcome_volume_w'] =\
        label_file_dfc.groupby(['year_month',
                                label_col])[weight_col].transform('sum')
        
      ### calculate weighted outcome rate per month & class
      if weights:
        label_file_dfc['outcome_rate_w'] =\
        label_file_dfc['outcome_volume_w'] / label_file_dfc['volume_w']\
        * 100
        
      ### create a summary table of counts by dropping duplicates
      ### & cleaning table
      if weights:
        df_counts =\
        label_file_dfc[['year_month', 'volume', 'volume_w', 'average_weight']].drop_duplicates().\
        sort_values(by = ['year_month'])

      else:
        df_counts =\
        label_file_dfc[['year_month', 'volume']].drop_duplicates().\
        sort_values(by = ['year_month'])
        
      ### create a summary table of class counts & outcome rates
      ### & cleaning table
      
      if weights:
        #### get pos outcome stats
        df_outcomes_pos =\
        label_file_dfc[label_file_dfc[label_col] ==\
                       outcome_class][['year_month',
                                       'outcome_volume',
                                       'outcome_volume_w',
                                       'outcome_rate',
                                       'outcome_rate_w']].\
        ...
        ### get a count of volumes
        label_file_df['volume'] = label_file_df.shape[0]
        
        ### get avg weight
        if weights:
          label_file_df['average_weight'] =\
          label_file_df[weight_col].mean()
          
        ### get a count of volumes per class
        label_file_dfc = label_file_df.copy() # copy to avoid warning
        label_file_dfc['outcome_volume'] =\
        label_file_dfc.groupby(label_col)[id_col].transform('count')
        
        ### calculate outcome rate per class
        label_file_dfc['outcome_rate'] =\
        label_file_dfc['outcome_volume'] / label_file_dfc['volume'] * 100
        
        ### get a count of weighted volumes & outcomes per class
        if weights:
          label_file_dfc['volume_w'] =\
          label_file_dfc[weight_col].sum()
          
          label_file_dfc['outcome_volume_w'] =\
          label_file_dfc.groupby(label_col)[weight_col].transform('sum')
          
        ### calculate weighted outcome rate per class
        if weights:
          label_file_dfc['outcome_rate_w'] =\
          label_file_dfc['outcome_volume_w'] / label_file_dfc['volume_w']\
          * 100
        
        ### create summary table of counts by dropping dups
        ### & cleaning table
        if weights:
          df_counts =\
          label_file_dfc[['volume', 'volume_w', 'average_weight']].drop_duplicates()
          
        else:
          df_counts = label_file_dfc[['volume']].drop_duplicates()
        
        ### create summary table of class counts & outcome rates
        if weights:
          #### get pos outcome stats
          df_outcomes_pos =\
          label_file_dfc[label_file_dfc[label_col] ==\
                         outcome_class][['outcome_volume',
                                         'outcome_volume_w',
                                         'outcome_rate',
                                         'outcome_rate_w']].\
          drop_duplicates()
    ...
    # format returned table
    ## reset indices
    df_counts.reset_index(drop = True, inplace = True)
    df_outcomes_pos.reset_index(drop = True, inplace = True)
    df_outcomes_neg.reset_index(drop = True, inplace = True)
    
    ## join df_outcomes & df_counts
    if monthly_f:
      df_summary = pd.merge(df_counts, df_outcomes_pos, how = 'left',
                            on = 'year_month')
      df_summary = pd.merge(df_summary, df_outcomes_neg, how = 'left',
                            on = 'year_month', suffixes = ['', '_neg'])
    
    else:
      df_summary = df_counts.join(df_outcomes_pos).join(df_outcomes_neg,
                                                        rsuffix = '_neg')
    ...
    if monthly_f:
      if weights:
        df_summary =\
        df_summary[['year_month', 'volume', 'volume_w',
                    'outcome_volume', 'outcome_volume_w',
                    'outcome_volume_neg', 'outcome_volume_w_neg',
                    'outcome_rate', 'outcome_rate_w',
                    'average_weight']].copy()

      else:
        df_summary =\
        df_summary[['year_month', 'volume', 'outcome_volume',
                    'outcome_volume_neg', 'outcome_rate']].copy()

    else:
      if weights:
        df_summary =\
        df_summary[['volume', 'volume_w',
                    'outcome_volume', 'outcome_volume_w',
                    'outcome_volume_neg', 'outcome_volume_w_neg',
                    'outcome_rate', 'outcome_rate_w',
                    'average_weight']].copy()
        ...
        df_summary =\
        df_summary[['volume', 'outcome_volume', 'outcome_volume_neg',
                    'outcome_rate']].copy()


    if time_var == True:
      print("label_file_summary: %s seconds" % (time.time() - start_time))
...
def weights(df, df_down_sampled, id_col = 'id', label_col = 'label',
            date_col = 'snap_d', monthly_f = False, time_var = False):
    # save function start time to calc function time later
    start_time = time.time()

    # create df_weighted such that df_down_sampled includes a weight column
    # ensure weights are calculated on a monthly basis
    if monthly_f:
        # create year month column in dfs
        df['month_year'] =\
        pd.to_datetime(df[date_col]).dt.to_period('M')
        df_down_sampled['month_year'] =\
        pd.to_datetime(df_down_sampled[date_col]).dt.to_period('M')
        
        # copy down sampled data frame
        final_df_down_sampled = df_down_sampled.copy()
                   
        # get pre down sampling control & target volumes per month
        df['total_v'] =\
        df.groupby([label_col, 'year_month'])[id_col].transform('count')
        
        # drop duplicates to summarise pre down sampling volumes
        df = df[['year_month', label_col, 'total_v']].drop_duplicates()
        
        # get post down sampling control & target volumes per month
        df_down_sampled['down_sampled_v'] =\
        df_down_sampled.groupby([label_col,
                                 'year_month'])[id_col].transform('count')
        
        # drop duplicates to summarise post down sampling volumes
        df_down_sampled =\
        df_down_sampled[['year_month', label_col,
                         'down_sampled_v']].drop_duplicates()
        
        # merge post & pre down sampling volumes per class, per month
        df_weighted = pd.merge(df, df_down_sampled,
                               on = [label_col, 'year_month'],
                               how = 'left').sort_values(by = [label_col,
                                                               'year_month'])
        
        # calculate weight column
        df_weighted['weight'] =\
        df_weighted['total_v'] / df_weighted['down_sampled_v']
        
        # copy summarised volume & weight table to return later
        weight_summary = df_weighted.copy()
        
        # merge weight column onto df_down_sampled
        df_weighted = pd.merge(final_df_down_sampled, df_weighted,
                               on = [label_col, 'year_month'], how = 'left')
        
        # simplify down sampled data frame with weights
        df_weighted = df_weighted[[id_col, label_col, date_col, 'weight']]
        
    # ensure weights are calculated on the whole population
    else:
      # copy down sampled data frame
      final_df_down_sampled = df_down_sampled.copy()
      
      # get total control & target volumes
        df['total_v'] = df.groupby(label_col)[id_col].transform('count')
        
        # drop duplicates to summarise pre down sampling volumes
        df = df[[label_col, 'total_v']].drop_duplicates()
        
        # get post down sampling control & target volumes per month
        df_down_sampled['down_sampled_v'] =\
        df_down_sampled.groupby(label_col)[id_col].transform('count')
        
        # drop duplicates to summarise post down sampling volumes
        df_down_sampled =\
        df_down_sampled[[label_col, 'down_sampled_v']].drop_duplicates()
    ...
    # give execution time of function if requested
    if time_var:
        print('weights: %s seconds' % (time.time() - start_time))
                   
    return df_weighted, weight_summary
...
def decile_stats(df, outcome_label = 1, score_col = 'label_1_prob',
    ...
    if print_f:
        print(str(round(decile_bands[decile_bands[decile_col] <=\
              ...
              '% of outcomes in top ' + str(decile_cut_off) + ' deciles\n')
...
def decile_plot(key_dict, outcome_rate_col = 'outcome_rate',
                decile_col = 'decile', title = 'Outcome Rate per Decile',
                ...
                bar_text_font_size = 15, time_var = False):
  '''
  key_dict = {'data_set': [db_train, db_test],
              'key': ['train', 'test'],
              'colour': ['#5543F2', '#E80A72']}
  '''
    # save function start time to calc function time later
    start_time = time.time()
    
    # combine all provided data sets & prepare for plotting
    for d in range(len(key_dict['data_set'])):
      # copy data frame
      temp_df = key_dict['data_set'][d].copy()
      
      # remove unnecessary columns
      temp_df = temp_df[[outcome_rate_col, decile_col]]
      
      # create key columns
      temp_df['key'] = key_dict['key'][d]
      
      # combine all data sets as df
      if d == 0:
        df = temp_df
    ...
    df['percent'] = round(df[outcome_rate_col] * 100, 2)

    # create base table
    decile_ls =\
    list(range(1, 11)) * len(key_dict['data_set'])
    key_ls = key_dict['key'] * 10
    base_df =\
    pd.DataFrame({decile_col: decile_ls,
                  'key': sorted(key_ls)})
    df = pd.merge(base_df, df, how = 'left',
                  on = [decile_col, 'key']).fillna(0)

    # create plot
    ax = sns.barplot(x = decile_col, y = 'percent', hue = 'key', data = df,
                     estimator = sum, ci = None, palette = key_dict['colour'])
